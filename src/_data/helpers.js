module.exports = {
   ...
   * @param {Boolean} random=true Wheter or not this should be randomised
  ...
  getSiblingContent(collection, item, limit = 3, random = true) {
    ...
    if (random) {
      ...
      while (counter > 0) {
        ...
        // Swap the last element with a random one
  ...
  filterCollectionByKeys(collection, keys) {
    return collection.filter((i) => keys.includes(i.data.key));
  },
};
